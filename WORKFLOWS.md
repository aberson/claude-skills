...
The [README](README.md) lists what each skill *is*; this page maps how they **chain together** in
...
flowchart LR
    subgraph PLAN
        A["plan-init (new project)<br>plan-feature (existing)"] --> B["plan-review"]
        B --> C["plan-wrap"]
    end
    ...
    subgraph BUILD
        E["build-phase"] --> F["build-step &times; N"]
        F --> G["review gates:<br>review-gauntlet / review-deep"]
    end
    subgraph SHIP
        H["acceptance:<br>review-uat / user-uat<br>user-walkthrough / user-shakedown"]
        I["repo-update"]
    end
    C --> D
    D --> E
...
```mermaid
flowchart LR
    S["skill-eval-setup<br>(bootstrap evals for a skill)"] --> I["skill-iterate<br>(overnight serial hill-climb)"]
    I -- "delta &lt; 1% = plateau" --> E["skill-evolve<br>(parallel A/B of N variants)"]
    I --> L["lesson-harvest<br>(draft un-codified lessons as a PR)"]
    L --> M["review-memories<br>(human gate: accept / adapt / reject)"]
```
